...
## 버전 기록
- 최신: **v0.5.0** 출석 관리 & 통계 대시보드 완성 + 배포 준비 (2026-02-10)
  - 출석 관리 기능 완성 (주간/월간 통계, 연속 출석)
  - 통계 대시보드 개선 (학습 진행률, 주간 트렌드, 성과 분석)
  - SEO 최적화 (메타데이터, sitemap, robots.txt)
  - PWA 지원 (manifest.json)
  - 배포 설정 완료 (Vercel)
- **v0.5.0** 일상 정보 페이지 (날씨 + 뉴스 통합) (2026-02-09)
  - [전체 요약](./docs/버전/v0.5.0-summary.md)
  - 실시간 날씨 정보 (OpenWeatherMap API)
  - 실시간 뉴스 정보 (네이버 뉴스 API)
  - 반응형 페이지네이션 (모바일/웹)
- **v0.2.0** 학습/할일/타임라인 기능 대폭 개선 (2026-02-03)
...
환경 변수
- **v0.5.0부터 실제 API 연동을 위한 환경 변수 필요**
- `.env.local` 파일 생성 (`.env.local.example` 참고)
- 필수 환경 변수:
  - `NEXT_PUBLIC_OPENWEATHER_API_KEY`: OpenWeatherMap API 키
  - `NEXT_PUBLIC_NAVER_CLIENT_ID`: 네이버 Client ID
  - `NEXT_PUBLIC_NAVER_CLIENT_SECRET`: 네이버 Client Secret
- 자세한 설정 방법은 [API 설정 가이드](./docs/API-SETUP.md) 참고

## 프로젝트 구조(Project Structure)
- app/
  - layout.tsx: 공통 레이아웃(헤더/본문/푸터). 헤더 우측 버튼은 MY↔MAIN 자동 전환
  - page.tsx: 메인 대시보드(학습 생성 + 통계 + 최근 학습 목록)
  - weather/page.tsx: 일상 정보 페이지(날씨 + 뉴스 통합, 반응형 페이지네이션) **[v0.5.0 신규]**
  - api/
    - weather/route.ts: OpenWeatherMap API 프록시 **[v0.5.0 신규]**
    - news/route.ts: 네이버 뉴스 API 프록시 (CORS 해결) **[v0.5.0 신규]**
  - mypage/
...
- components/
  ...
  - ui/: Card, Progress, Badge, Checkbox, Separator, Button 등 UI 프리미티브 **[Button 추가]**
...
- lib/
  ...
  - api/weather.ts: 날씨 API 호출 함수 **[v0.5.0 신규]**
  - api/news.ts: 뉴스 API 호출 함수 **[v0.5.0 신규]**
  - hooks/useMediaQuery.ts: 미디어 쿼리 감지 훅 **[v0.5.0 신규]**
  - utils.ts: `cn` 헬퍼
- docs/
  - API-SETUP.md: API 키 발급 및 설정 가이드 **[v0.5.0 신규]**
  - 버전/: 버전별 상세 문서(v0.1.0 ~ v0.5.0)
  - 정리/
    - learning-path.md: 학습 경로 가이드
    - flow.md: 플로우 차트
    - architecture/: 아키텍처 문서
- .env.local.example: 환경 변수 템플릿 **[v0.5.0 신규]**
...
- **[v0.5.0]** Next.js API Routes로 CORS 문제 해결
- **[v0.5.0]** 실시간 API 연동 (OpenWeatherMap, 네이버 뉴스)
- **[v0.5.0]** 반응형 페이지네이션 (모바일/웹 자동 전환)
- **[v0.5.0]** 환경 변수로 안전한 API 키 관리
